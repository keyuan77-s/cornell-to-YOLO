...
def convert(src_path, target_path):
    img_path = "{}/images".format(src_path)
    label_path = "{}".format(src_path)
    ...
    for name in img_name:
        ...
        with open(os.path.join(target_path, name + "r.txt"), 'w') as f:
            ...
            with open(labelpath, 'r', encoding='utf-8-sig') as fl:
                ...
                ima = pd.DataFrame(image_box)
                ima = ima.dropna(axis = 0,how='any')
                image_box = np.array(ima).astype(np.int32)
                print(image_box)
                ...
                for i, elm in enumerate(boxes):
                    ...
                    sit = math.sin(angle/ 180 * math.pi)
                    cot = math.cos(angle/ 180 * math.pi)
                    angles.append([angle])
                    sis.append([sit])
                    coss.append([cot])
                ...
                xywhcs = np.hstack((xywh, coss, sis))
                for box in xywhcs:
                    cx = box[0]
                    cy = box[1]
                    w = box[2]
                   ...
                   # a = a / 180 * math.pi
                    ...
                    si = Decimal(si).quantize(Decimal('0.000000'))
                    co = Decimal(co).quantize(Decimal('0.000000'))
...
if __name__ == '__main__':
    convert("E:/grasp-detection/covert/Cornell-grasp-detection-box-master/Cornelldataset/Cornell", "E:/grasp-detection/covert/Cornell-grasp-detection-box-master/Cornelldataset/xywhcs")